...
void matvec_serial(const double *L, const double *x, double *y, int D) {
    for (int i = 0; i < D; ++i) {
        double sum = 0.0;
...
void vec_axpy(double alpha, const double *x, double *y, int D) {
    for (int i = 0; i < D; ++i) {
        y[i] += alpha * x[i];
    }
}

void vec_copy(const double *x, double *y, int D) {
    for (int i = 0; i < D; ++i) {
        y[i] = x[i];
    }
}

void rk4_step_serial(const double *L,
                     double *r,
                     double *k1,
                     double *k2,
                     double *k3,
                     ...
                     double dt) {
    /* k1 = L r */
    matvec_serial(L, r, k1, D);

    /* tmp = r + dt/2 * k1 */
    vec_copy(r, tmp, D);
    vec_axpy(0.5 * dt, k1, tmp, D);
    matvec_serial(L, tmp, k2, D);

    /* tmp = r + dt/2 * k2 */
    vec_copy(r, tmp, D);
    vec_axpy(0.5 * dt, k2, tmp, D);
    matvec_serial(L, tmp, k3, D);

    /* tmp = r + dt * k3 */
    vec_copy(r, tmp, D);
    vec_axpy(dt, k3, tmp, D);
    matvec_serial(L, tmp, k4, D);
...
int main(int argc, char **argv) {
    ...
    for (int n = 0; n < nsteps; ++n) {
        rk4_step_serial(L, r, k1, k2, k3, k4, tmp, D, dt);
        t += dt;
    }
